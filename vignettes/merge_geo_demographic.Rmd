...
Now, the `merge_geo_demographic()` function only takes by parameter the demographic dataset of interest.  Therefore, we should first access the demographic documentation to know which dataset we want to work with. Let's suposse we want to select a dataset at the deparment level.
:::
...
ggplot(data = merged_data) +
  ...
  ggtitle(
    label = "Proportion of people who reported using home remedies to treat a health problem",
    subtitle = "Colombia"
  )
```
