...
::: {style="text-align: justify;"}
Climate data is not stored in multiple datasets but as an unique dataset with numerous tags. These tags can also be consulted through the function `climate_tags()`.
:::

```{r dicionary for climate data}
dict_climate <- climate_tags()
head(dict_climate)
```
