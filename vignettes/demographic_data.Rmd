...
lit <- literacy %>% subset(municipio == "Villapinzón" & grupo_de_edad=="entre_15_y_24_anos"& area=="total" &sabe_leer_y_escribir=="total"&sexo=="total")%>%select(municipio, area, grupo_de_edad, sexo, total)
head(lit)

# Filter of marital status data to have a count of population under specific conditions
ms <- marital_status %>% subset(municipio == "Villapinzón" & area=="total" & grupo_de_edad==c("15_a_19", "20_a_24")&estado_civil=="total"&sexo=="total") %>% select(grupo_de_edad, estado_civil, sexo, total)
...
lit_filt <- literacy %>% subset(municipio == "Villapinzón" & sabe_leer_y_escribir=="no" & grupo_de_edad=="entre_15_y_24_anos"& area=="total" )%>%select(sabe_leer_y_escribir, grupo_de_edad, sexo, total)%>%  mutate(estado = "no_sabe_leer_y_escribir") %>% select(-sabe_leer_y_escribir)  %>%
...
ms_filt15 <- marital_status %>% subset(municipio == "Villapinzón" & area=="total" & grupo_de_edad=="15_a_19") %>% select(grupo_de_edad, estado_civil, sexo, total) 
...
ms_filt20 <- marital_status %>% subset(municipio == "Villapinzón" & area=="total" & grupo_de_edad=="20_a_24") %>% select(grupo_de_edad, estado_civil, sexo, total) 
...
ms_final<- ms_filt %>% 
  group_by(estado_civil, sexo) %>%
  dplyr::summarise(total = sum(total)) %>%    ungroup()%>%
  mutate(grupo_de_edad = "entre_15_y_24_anos") %>%     rename(estado = estado_civil)  %>% 
  mutate(proportion = total / lit$total)

# Merge marital status and literacy final dataframes, with some subsets to plot a better graph
merged_df <- bind_rows(lit_filt, ms_final) %>% subset(sexo!="total") %>% subset(estado!="total") %>% subset(estado!="soltero_a")  
...
ggplot(merged_df, aes(x = estado, y = proportion, fill = sexo)) +
  ...
  labs(title = "Proportion of people in Villapinzón according to marital status and lack of writing and reading knowledge, by sex",
       x = "Status",
       y = "Count",
       fill = "Sex") +  theme_minimal() +
