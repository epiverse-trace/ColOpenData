...
```{r data filtering and plot, echo=TRUE}

# Subset data so it presents the information regarding the water supply system, by department
wss <- public_services_d %>% subset (area=="total_departamental"&servicio_publico=="acueducto") %>%  select(departamento, disponible, total)

# Calculate the total counts for each department (for all situations)
total_counts <- wss %>% group_by(departamento) %>% summarise(total_all = sum(total)) %>%  ungroup()

# Calculate the proportion of counts of "si" by department
proportions <- wss %>% filter(disponible == "si") %>% left_join(total_counts, by = "departamento") %>%
...
ggplot(proportions, aes(x = reorder(departamento, -proportion_si), y = proportion_si)) +
  ...
  labs(title = "Proportion of dwellings with access to water supply system, by department",
       x = "Department",
       y = "Proportion") +
  theme_minimal() +
