...
**ColOpenData** can be used to access open demographic data from Colombia. This demographic data is retrieved from the National Administrative Department of Statistics ([DANE](https://www.dane.gov.co/index.php/estadisticas-por-tema), for its acronym in Spanish). The demographic module allows you to consult demographic data from the National Population and Dwelling Census (CNPV) of 2018.

The available CNPV information is divided in four categories: households, persons demographic, persons social and dwellings. All data documentation can be accessed as explained at [Documentation and Dictionaries](https://epiverse-trace.github.io/ColOpenData/articles/documentation_and_dictionaries.html). 
...
lit <- literacy %>%
  subset(municipio == "Villapinz<U+00F3>n" & grupo_de_edad == "entre_15_y_24_anos" &
...
ms <- marital_status %>%
  subset(municipio == "Villapinz<U+00F3>n" & area == "total" &
...
lit_filt <- literacy %>%
  subset(municipio == "Villapinz<U+00F3>n" & sabe_leer_y_escribir == "no" &
...
ggplot(merged_df, aes(x = estado, y = proportion, fill = sexo)) +
  ...
  labs(
    title = "Proportion of people in Villapinz<U+00F3>n by marital status and
    iliteracy by sex",
    x = "Status",
    y = "Count",
    fill = "Sex"
