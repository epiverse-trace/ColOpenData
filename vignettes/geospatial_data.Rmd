...
For this example we will be using geospatial data at the level of Urban Section (SECU) for the city of Tunja, Colombia. We will be using the demographic data included in the dataset to generate visualizations using **ggplot2** and **leaflet**.

First, we download the data using the function `download_geospatial`, including the geometries and the CNPV related information. After downloading, we have to filter by the municipality code using the DIVIPOLA code for Tunja, which is `"15001"`. For further information on DIVIPOLA please consult the dataset [here](https://www.datos.gov.co/widgets/gdxc-w37w).
