...
vignette: >
  %\VignetteIndexEntry{geospatial_data}
...
**ColOpenData** can be used to access open geospatial data from Colombia. This 
...
[(DANE)](https://www.dane.gov.co/index.php/en/).
...
dataset to consult. The available datasets are stored in the same pattern:

::: {.alert .alert-info}
  SOURCE_DATASET_YEAR_DETAILS
:::

All datasets contained come from the same source: DANE, and are the same type
of dataset: MGNCNPV which contains the National Geostatistical Framework
including the summarized National Population and Housing Census of 2018. Details
for geospatial datasets relate to the level of aggregation as follows:

```{r Available datasets, echo = FALSE}
code <- c(
  "DANE_MGNCNPV_2018_DPTO", "DANE_MGNCNPV_2018_MPIO", "DANE_MGNCNPV_2018_MPIOCL",
  "DANE_MGNCNPV_2018_MZN", "DANE_MGNCNPV_2018_SECR", "DANE_MGNCNPV_2018_SECU",
...
and visualize the municipalities with the highest case count.
...
To download the geospatial data we will use the `download_geospetial` function,
with the respective code for municipality level.
...
str(departments)
```
Once downloaded, we can subset the data by regions or columns of interest. 
In this example we only need the departments codes and geometries.
