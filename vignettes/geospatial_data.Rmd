...
`download_geospatial` requires three arguments:
...
In this vignette, we will download the geospatial data and provide some examples on how to use the available information and visualize it using graphic libraries like **ggplot2** and **leaflet**. We will be using geospatial data at the level of Urban Section (SECU) for the city of Tunja, Colombia and will calculate the percentage of houses with internet connection at each Urban Section.
...
To calculate the percentage of houses with internet connection, we will need to know the number of houses with internet connection and the total of houses in each Urban Section. From the dictionary we get that the number of houses with internet connection is [STP19_INT1]{.underline} and the total of houses is [STVIVIENDA]{.underline}. We will calculate the percentage as follows:
