...
After exploring the dictionary, we can identify the column that contains the individual municipality codes, which is *MPIO_CDPMP*. This column contains the concatenated code for department and municipality, which is an unique identifier for every municipality.

We will be calculation the percentage of houses with internet connection at each urban section. For this purpose we will need to know the number of houses with internet connection and the total of houses in each section. From the dictionary we get that the number of houses with internet connection is *STP19_INT1* and the total of houses is *STVIVIENDA*. We will calculate the percentage as follows:
