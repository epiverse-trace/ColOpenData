...
In this vignette you will learn how to use **{ColOpenData}** to:
1. Download climate data from:
  - previously selected stations using the `download_climate_stations()` function,
  - a specified geometry (ROI) using the `download_climate_geom()` function,
  - municipalities' or departments' already loaded geometries using the `download_climate()` function.
2. Aggregate climate data by different frequencies
3. Plot downloaded climate data

The example below will show how to download Maximum Temperature data (TMX_CON) between 2013 and 2016 from  the municipality of Manizales in Colombia. This data will highlight the increase in temperatures observed during 2015 and 2016 due to the impact of El Nino (ENSO).
...
::: {style="text-align: justify;"}
We will need to create a spatial polygon around the municipality of Manizales and use that as our ROI to retrieve the climate data. The GPS coordinates of the geometry are needed to create the spatial polygon. For simplicity, we will build a bounding box with the four points which bound the municipality, and transform the created geometry into an `sf` object (see [**sf**](https://r-spatial.github.io/sf/articles/sf1.html) library for further details).
:::
...
With our created ROI, we can make a simple visualization using `leaflet()` from the {leaflet} package.
...
::: {style="text-align: justify;"}
As we can see, only two stations have data for the selected period, and we have missing information in some periods. Additionally, by having the data daily we cannot easily see big changes over time. To increase the resolution of the plot, we will use the aggregation function `aggregate_climate`, which aggregates climate data by time. This function aggregates the parameter of interest based on the chosen period.
:::
