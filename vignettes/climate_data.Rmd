...
From the original 129 stations, 40 were working for some or the whole period of interest and collected information for Dry-bulb temperature. It's important to consider that after data collection, some information might be lost due to quality attributes. 
With this information, we can access the Dry-bulb temperature from 2013 to 2016. To do so, we can use the function `download_climate_stations`. This function has the following parameters:
...
date, hour, tag requested and value recorded at the specified time. The structure reports a row for each observation for value.
...
ggplot(data = max_temperature_stations) +
  ...
  ggtitle("Max Temperature in Manizales") +
  xlab("Date") +
  ylab("Temperature [°C]") +
  theme_bw() +
  facet_grid(rows = vars(station))
```
As we can see, only two stations have data for the selected period, and we have missing information in some periods. Additionally, by having the data hourly we cannot easily see big changes along time. To aid this issue, we will use an aggregation function. Aggregation functions are available at **ColOpenData** for climate data. since we are dealing with max temperature, we will use `aggregate_tmx()`, adding as parameter the desired aggregation.

```{r}
max_temperature_month <- max_temperature_stations %>% aggregate_tmx("month")
...
We will use the same ROI created for the previous example, and add the aggregation for month.
...
ggplot(data = max_temperature_mpio) +
  ...
  facet_grid(rows = vars(station))
```
The three methods show the same results, and the increase in maximum monthly temperature can be perceived during 2015 and 2016.

### Disclaimer

Data availability is subdued to station measurements and quality filters. In most cases this leads to a lower amount of data, considering the extensive amount of climate stations.

Aggregation is only available for tags that are explicitly reported by IDEAM, including the corresponding methodology for aggregation. The daily, monthly and annual aggregation is available for:
