---
title: "Extracting climate data"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{climate_data}
...
For this vignette we will retrieve data for the city of Manizales in Colombia. 
We will download Dry-bulb temperature from 2013 to 2016, to evaluate the
increase in the temperature during 2015 and 2016 thanks to the impact of El 
Niño (ENSO).
...
function `download_climate`, which downloads data from climate stations 
...
observations, however, we still can access their historical data.

Now that we confirmed that there are meteorological stations in the area, we
will access the Dry-bulb temperature from 2013 to 2016. To do this we will use
`download_climate`.
...
ggplot(data = db_temperature_roi) +
  ...
  xlab("Date") +
  ylab("Dry-bulb temperature [<U+00B0>C]") +
  theme_bw()
```
...
geometries like municipalities, `ColOpenData` offers a second function, to
...
following table[(table)](https://www.datos.gov.co/widgets/gdxc-w37w). To
replicate our previous example we need the code for Manizales, which is "17001".

The function `download_climate_mpio` will require almost the same arguments 
than `download_climate` as follows:
- `name`: Character with the DIVIPOLA code for the municipality.
- `start_date`: Character with the first date to consult in format "YYYY-MM_DD".
- `end_date`: Character with the last date to consult in format "YYYY-MM_DD".
- `frequency`: Character with the aggregation frequency. Can be "day", "week",
"month" or"year".
...
```{r download climate by municipality}
