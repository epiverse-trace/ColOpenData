...
- `aggregate`: Boolean for data aggregation, if FALSE, returns the data from each
...
observations, however, we still can access the historical data. To understand the category of each station we can use the definition catalog provided by DANE [here](http://www.ideam.gov.co/documents/10182/557765/Definiciones+CNE.pdf/25e1cca5-ee47-4eaf-86c0-c4f5192a9937). For this example, stations under the categories *Climática Principal* and *Climática Secundaria* have records of Dry-bulb temperature.
...
geometries like municipalities, **ColOpenData** offers an extra function to
...
than `download_climate`, but replace the `sf` object for a character containing the DIVIPOLA code:
