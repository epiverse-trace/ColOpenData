...
#'
#' @examples
#' merged <- merge_geo_dem ("department", "DANE_CNPVPD_2018_4PD", 
...
merge_geo_dem <- function(spatial_level, dem_dataset, column) {
  ...
  cols_exclude <- c(total_col, column, "codigo_departamento", "departamento",
  ...
  merged_df <- merge(geo_dpto, filtered_df, by.x = "DPTO_CCDGO", 
    ...
    merged_df <- merge(geo_mpio, filtered_df, by.x = "MPIO_CDPMP",
