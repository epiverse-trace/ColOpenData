...
test_that("Filter MGN-CNPV errors are thrown", {
  ...
  expect_error(filter_mgn_cnpv(.data = dataset_2, 
                               level = 2, 
                               filter = list(
                                 AD = c("05", "73"),
                                 columns = c("1", "THIS")
...
test_that("Filter MGN-CNPV works as expected", {
  expect_s3_class(filter_mgn_cnpv(.data = dataset_1, 
                                  level = 1, 
                                  include_geometry = FALSE), 
                  "data.frame")
  expect_s3_class(filter_mgn_cnpv(.data = dataset_2, 
                                  level = 2, 
                                  include_geometry = FALSE), 
                  "data.frame")
  expect_s3_class(filter_mgn_cnpv(.data = dataset_1, 
