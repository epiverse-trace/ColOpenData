...
test_that("Filter MGN-CNPV works as expected", {
  ...
  expect_s3_class(filter_mgn_cnpv(.data = dataset_2, level = 2, include_geometry = FALSE), "data.frame")
  expect_s3_class(filter_mgn_cnpv(.data = dataset_1, level = 1), "data.frame")
  expect_s3_class(filter_mgn_cnpv(.data = dataset_2, level = 2), "data.frame")
  expect_s3_class(filter_mgn_cnpv(.data = dataset_1, level = 1, filter = list(AD = c("CUNDINAMARCA", "META"))), "data.frame")
  expect_s3_class(filter_mgn_cnpv(.data = dataset_2, level = 2, filter = list(AD = c("RIONEGRO", "MARINILLA"))), "data.frame")
})
